...
function labels(){pg.innerHTML=lg.innerHTML="";ORDER.forEach((k,i)=>{const q=lang.qcs[k],mid=start+(i+.5)*stepDeg,bot=((mid%360)+360)%360>0&&((mid%360)+360)%360<180,lines=q.lines.slice(0,4),c=(lines.length-1)/2;lines.forEach((line,j)=>{const off=(c-j)*34*(lang.labelLineSpacing??1),r=labelR+(bot?-off:off),id=`ca${i}_${j}`,path=document.createElementNS("http://www.w3.org/2000/svg","path");path.id=id;path.setAttribute("d",arc(r,start+i*stepDeg+3.5,start+(i+1)*stepDeg-3.5,bot));pg.appendChild(path);const t=document.createElementNS("http://www.w3.org/2000/svg","text");t.setAttribute("class",line.emphasis?"arc-label-adj":"arc-label-noun");t.setAttribute("font-size",(line.emphasis?41.5:33.5)*(lang.labelScale??1));const tp=document.createElementNS("http://www.w3.org/2000/svg","textPath");tp.setAttribute("href","#"+id);tp.setAttribute("startOffset","50%");tp.setAttribute("text-anchor","middle");tp.textContent=line.text;t.appendChild(tp);lg.appendChild(t)})})}
function ensureWedges(){if(wg.children.length)return;ORDER.forEach((_,i)=>{const x=document.createElementNS("http://www.w3.org/2000/svg","path");x.classList.add("wedge","compare-wedge");x.dataset.i=i;x.setAttribute("fill",grad(i));wg.appendChild(x)})}
