(() => {
  ...
  function chooseRecordingFormat(){
    const mp4=["video/mp4;codecs=h264","video/mp4;codecs=avc1.42E01E","video/mp4;codecs=avc1","video/mp4"],m4=mp4.find(t=>MediaRecorder.isTypeSupported(t));
  ...
  function makeLabelLayer(sourceSvg){const layer=document.createElement("canvas");layer.width=SIZE;layer.height=SIZE;const lctx=layer.getContext("2d");lctx.imageSmoothingEnabled=true;lctx.imageSmoothingQuality="high";lctx.globalAlpha=1;lctx.globalCompositeOperation="source-over";drawCanvasLabels(sourceSvg,lctx);return layer;}
  async function drawSvgFrame(svg,ctx,labelLayer){const xml=new XMLSerializer().serializeToString(prepareSvgClone(svg)),blob=new Blob([xml],{type:"image/svg+xml;charset=utf-8"}),url=URL.createObjectURL(blob);try{const im=new Image();im.src=url;await im.decode();ctx.fillStyle="#fff";ctx.fillRect(0,0,SIZE,SIZE);ctx.drawImage(im,0,0,SIZE,SIZE);if(labelLayer){ctx.save();ctx.globalAlpha=1;ctx.globalCompositeOperation="source-over";ctx.drawImage(labelLayer,0,0);ctx.restore();}}finally{URL.revokeObjectURL(url);}}
